...
cameraProcess = sp.Popen(videoCmd, stdout=sp.PIPE, stderr=sp.DEVNULL) # start the camera
...
while True:
    ...
    start_time = time.process_time() # Changed from time.clock() to time.process_time()
    ...
    if len(points[0]) > 0:
        ...
        for altura in range(start_height - 1, 0, -step):
            if(thresh[altura][last_x] == 255): # if the pixel is white it means the line turned right
    ...
    print("Loop took:", str((time.process_time()- start_time) * 1000), 'ms') # Changed from time.clock() to time.process_time()
